...
import { addPhotoSchema } from '../utils/validationSchemas';
...
export const photoController = {
  ...
  addPhoto: async (req: Request, res: Response) => {
    try {
      ...
      const photoData = addPhotoSchema.parse(req.body);
      const newPhoto = await photoService.addPhoto(Number(inspectionId), photoData.url);
   ...
   *         description: Erro interno do servidor.
   */
  getPhotosByInspectionId: async (req: Request, res: Response) => {
    try {
      const { inspectionId } = req.params;
      // Optionally, check if inspection exists before fetching photos
      const photos = await photoService.getPhotosByInspectionId(Number(inspectionId));
